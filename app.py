...
def token_obrigatorio(f):
    ...
    def decorated(*args,**kwargs):
        ...
        try:
            resutado = jwt.decode(token, app.config['SECRET_KEY'],algorithms=['HS256'])
            autor = Autor.query.filter_by(id_autor=resutado['id_autor']).first()
        except:
            return jsonify({'Mnesagem':'Token invalido'},401)
